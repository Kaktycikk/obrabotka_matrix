...
def is_valid_hex_number(num_str):
    ...
    # Используем регулярное выражение для проверки формата числа
...
def convert_to_words_hex(num_str):
    ...
    # Используем регулярное выражение для разбиения числа на отдельные символы
    ...
    # Преобразуем каждый символ в слово
    return sign + ' '.join(digit_to_word_hex[digit] for digit in digits)

def process_file(filename):
    with open(filename, 'r', encoding='utf-8') as file:
        # Используем регулярное выражение для разделения текста на объекты
        objects = re.split(r'\s+', file.read().strip())
        # Фильтрация с использованием проверки через регулярное выражение
...
filename = 'text.txt'  # Убедитесь, что файл 'text.txt' существует и содержит тестовые данные
