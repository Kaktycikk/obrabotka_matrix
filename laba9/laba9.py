...
def start_game_X():  # Начало игры для крестиков
    ...
    for i in range(3):
        for j in range(3):
            buttons[i][j] = tk.Button(game_window, text="", width=button_width, height=button_height, font=("Arial", 24), command=lambda i=i, j=j: player_turn(i, j))
...
def start_game_O():  # Функция для начала игры с ноликами
    ...
    for i in range(3):  # Создание кнопок для игрового поля
        for j in range(3):
            buttons[i][j] = tk.Button(game_window, text="", width=button_width, height=button_height, font=("Arial", 24),
...
center_window(start_game_window, 400, 200)  # Центрируем окно выбора
